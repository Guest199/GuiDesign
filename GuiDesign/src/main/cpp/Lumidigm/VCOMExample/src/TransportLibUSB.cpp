...
#include "../include/TransportLibUSB.h"
...
libusb_device_handle * TransportUSB::AndroidInitialize(V100_DEVICE_TRANSPORT_INFO* pTransport)
{
	...
	for (ssize_t i = 0; i < count; i++)
	{
		...
		if (!libusb_get_device_descriptor(apDevices[i], &desc))
		{
		    ...
		    { m_EPDown = V30X_ENDPOINT_DOWN; m_EPUp = V30X_ENDPOINT_UP; countFound++; deviceFound = apDevices[i]; }
		    else if (desc.idVendor == TransportUSB::M30X_VENDOR && desc.idProduct == TransportUSB::M30X_PRODUCT)
		    { m_EPDown = M30X_ENDPOINT_DOWN; m_EPUp = M30X_ENDPOINT_UP; countFound++; deviceFound = apDevices[i]; }
		}
	}
...
uint32_t TransportUSB::Initialize(V100_DEVICE_TRANSPORT_INFO* pTransport)
{
	...
	libusb_device_handle * h = NULL;
		 ...
		 { m_EPDown = M30X_ENDPOINT_DOWN; m_EPUp = M30X_ENDPOINT_UP; }
	else if ((h = libusb_open_device_with_vid_pid(m_libusb_context, V30X_VENDOR, V30X_PRODUCT)))
		 { m_EPDown = V30X_ENDPOINT_DOWN; m_EPUp = V30X_ENDPOINT_UP; }
#endif
																				      ...
																				      uint8_t * pResponse, uint32_t &nRxSize)
{
	...
	m_USBCB_request.ulCommand = 0x00;
	m_USBCB_request.ulCount	  = nTxSize;
	m_USBCB_request.ulData	  = 0x0;
	...
	if (pTransferReadUSBCB && pTransferReadPayload && pTransferWrite)
	{
		...
		libusb_fill_bulk_transfer(pTransferReadUSBCB, m_libusb_device_handle, m_EPUp,
								  (unsigned char *) &m_USBCB_response, sizeof(m_USBCB_response),
								  usbcb_read_callback, this, 15 * 1000/*msto*/);
		libusb_fill_bulk_transfer(pTransferReadPayload, m_libusb_device_handle, m_EPUp,
								  pResponse, nMaxRx,
								  payload_read_callback, this, 15 * 1000/*msto*/);
		libusb_fill_bulk_transfer(pTransferWrite, m_libusb_device_handle, m_EPDown,
			...
			if (!rc_libusb)
			{
                ...
                if (!rc_libusb)
				{
					...
					do
					{
						...
						if (rc_poll != 0)
						{
							DumpLogFmt(logFilename, "TransportUSB::TransmitCommand: poll: %d.\n", // (%s).\n",
								rc_poll); //, libusb_error_name(rc_poll));
							m_bError = true;
							break;
						}
				...
				{
					DumpLogFmt(logFilename, "TransportUSB::TransmitCommand: submit write USBCB failed %d.\n", rc_libusb);
				}
			}
			else
			{
				DumpLogFmt(logFilename, "TransportUSB::TransmitCommand: submit read payload failed %d.\n", rc_libusb);
			}
		}
		else
		{
			DumpLogFmt(logFilename, "TransportUSB::TransmitCommand: submit read usbcb failed %d.\n", rc_libusb);
		}
	}
	else
	{
