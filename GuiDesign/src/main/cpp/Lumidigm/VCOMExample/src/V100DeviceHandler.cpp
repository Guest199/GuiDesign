...
static int DLOpenSEngine()
{
	...
	if (DLSEngineHandle)
		return 0;
	DLSEngineHandle = dlopen("/usr/local/Lumidigm/lib/libSEngine.so", RTLD_LAZY);
	if (DLSEngineHandle)
	{
...
V100_ERROR_CODE V100DeviceHandler::GetDevice(uint deviceNumber, Device* pDevice)
{
	...
	if (bRC)
	{
		...
		{
			pDevice->Init(deviceNumber, V100_NORMAL_DEVICE, (uchar*) "COMM-PIPE-DUMMY", (uchar*) "M30X", 0, MERCURY_30X);
		...
		{
			pDevice->Init(deviceNumber, V100_NORMAL_DEVICE, (uchar*) "COMM-PIPE-DUMMY", (uchar*) "V30X", 0, VENUS_NS);
		...
		else
		{
			...
			if (pcType)
			{
				if (!DLOpenSEngine())
				{
					pDevice->Init(deviceNumber, V100_STREAMING_DEVICE, (uchar*) "COMM-PIPE-DUMMY", (uchar*) "V31X", 0, VENUS_31X);
					char * argv[13] =
									{
